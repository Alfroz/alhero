     ...
     */
    export function createDocumentWithID(db, collectionName, docId, data) {
      return db.collection(collectionName).doc(docId).set(data).then(() => true).catch(function (error) {
        return false;
      });
    }
    /**
    ...
    export function createNewDocument(db, collectionName, data) {
      return db.collection(collectionName).add(data).then(function (docRef) {
        return docRef;
    ...
    */
    ...
    export function updateDocument(db, collectionName, docId, data) {
      return db.collection(collectionName).doc(docId).update(data).then(() => true).catch(err => err);
    }
    /**
     * Delete a document
     *
    ...
    export function deleteDocument(db, collectionName, docId) {
      return db.collection(collectionName).doc(docId).delete().then(() => {
        return {
          status: true,
          message: `${docId} successfully deleted!`
        };
      }).catch(error => {
        return {
          status: true,
    ...
    */
    ...
    export function deleteDocumentFromSubcollection(db, collectionName, docId, collectionNameTwo, docIdTwo) {
      db.collection(collectionName).doc(docId).collection(collectionNameTwo).doc(docIdTwo).delete().then(() => {
    ...
    */
    ...
    export function checkDocumentExists(db, collectionName, docId) {
    ...
    export function getDocument(db, collectionName, documentId) {
    ...
    export function queryData(db, collectionName, queryArray, orderBy = null) {
    ...
    export function backup(db, collectionName, subCollection = '') {
      // console.log('Getting data from: ', collectionName);
      return new Promise((resolve, reject) => {
        const data = {};
        data[collectionName] = {};
        const results = db.collection(collectionName).get().then(snapshot => {
          snapshot.forEach(doc => {
    ...
    export function getSubCollection(db, data, dt, collectionName, subCollection) {
    ...
    export function addSubCollection(db, key, subData, collectionName, subCollection) {
      return new Promise((resolve, reject) => {
        db.collection(collectionName).doc(key).collection(subCollection).get().then(snapshot => {
    ...
    export function restore(db, fileName) {
      const that = this;
      return new Promise((resolve, reject) => {
    ...
    export function updateCollection(db, dataArray) {
      return __awaiter(this, void 0, void 0, function* () {
    ...
    export function startUpdating(db, collectionName, doc, data) {
      return new Promise(resolve => {
        return db.collection(collectionName).doc(doc).set(data).then(() => {
